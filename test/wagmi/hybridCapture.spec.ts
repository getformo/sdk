...
describe("wagmi hybrid capture", () => {
  ...
  it("clears the ended session even when the disconnect is dropped by the lock", async () => {
    // A disconnect that lands while status processing is locked is
    // dropped, not queued. The fallback pair must be cleared anyway, or
    // the rapid reconnect's chain report lands on the old provider.
    const bare = () => ({
      on: () => undefined,
      removeListener: () => undefined,
      request: async ({ method }: { method: string }) =>
    ...
    // Hold the processing lock the way a concurrent transition would.
